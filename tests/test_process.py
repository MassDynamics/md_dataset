...
def test_run_process_uses_config(input_data_sets: list[InputDataset], input_params: TestBlahParams, \
    ...
    @md_py
    def run_process_config( input_data_sets: list[InputDataset], params: TestBlahParams, \
...
def test_run_process_sets_name_and_type(input_data_sets: list[InputDataset], input_params: TestBlahParams, \
        fake_file_manager: FileManager):
    @md_py
    def run_process_sets_name_and_type(input_data_sets: list[InputDataset], input_params: InputParams, \
            output_dataset_type: DatasetType) -> dict: # noqa: ARG001
    ...
    results = run_process_sets_name_and_type(input_data_sets, input_params, DatasetType.INTENSITY)
...
def test_run_process_sets_flow_output(input_data_sets: list[InputDataset], input_params: TestBlahParams, \
        fake_file_manager: FileManager):
    @md_py
    def run_process_sets_flow_output(input_data_sets: list[InputDataset], input_params: InputParams, \
            output_dataset_type: DatasetType) -> dict: # noqa: ARG001
        return {
    ...
    results = run_process_sets_flow_output(input_data_sets, input_params, DatasetType.INTENSITY)
